...
def proxy(path):
	try:
		...
		if mimetype == "text/html" and os.getenv("REPLACE_CALLBACK", "") != "":
			try:
				i = __import__(os.getenv("REPLACE_CALLBACK").split(":")[0])
				content = getattr(i, os.getenv("REPLACE_CALLBACK").split(":")[1])(content, path)
			except:
				pass

		return Response(content, headers = headers, status = r.status_code, mimetype=mimetype)
	except:
